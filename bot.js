/**
 * Video Traffic Lab v1.0
 * ─────────────────────────────────────────────────────
 * Bot that:
 *  1. Opens your TikTok-like video site
 *  2. Watches videos until they end
 *  3. Moves to next video
 *  4. Exits randomly after 1, 2, 3, 6, ... videos
 */
'use strict';
...
const CFG = {
    ...
    videoWaitBuffer:    parseInt(process.env.VIDEO_WAIT_BUFFER || "3", 10), // seconds extra after video ends
    maxVideoDuration:   parseInt(process.env.MAX_VIDEO_DURATION || "90", 10), // cap to avoid infinite wait
    nextVideoMethod:    process.env.NEXT_VIDEO_METHOD   || "auto" // auto | scroll | button
};
...
const TZ_OFFSETS = {
    'America/New_York': 300, 'America/Chicago': 360, 'America/Denver': 420, 'America/Phoenix': 420, 'America/Los_Angeles': 480,
...
function buildStealthScript(fp) {
    const cfg = JSON.stringify({
        ua: fp.userAgent, platform: fp.platform, languages: fp.languages, cores: fp.cores, memory: fp.memory,
        screen: fp.screen, colorDepth: fp.colorDepth, gpuVendor: fp.gpuVendor, gpuRenderer: fp.gpuRenderer,
    ...
    (function () {
        ...
        try {
            ...
            const pdf1 = mkMime('application/pdf', 'pdf', 'PDF'); const pdf2 = mkMime('text/pdf', 'pdf', 'PDF');
            const plugins = [ mkPlugin('PDF Viewer', 'internal-pdf-viewer', 'PDF', [pdf1, pdf2]), mkPlugin('Chrome PDF Viewer', 'internal-pdf-viewer', 'PDF', [pdf1, pdf2]), mkPlugin('Chromium PDF Viewer', 'internal-pdf-viewer', 'PDF', [pdf1, pdf2]), mkPlugin('Microsoft Edge PDF Viewer', 'internal-pdf-viewer', 'PDF', [pdf1, pdf2]), mkPlugin('WebKit built-in PDF', 'internal-pdf-viewer', 'PDF', [pdf1, pdf2]), mkPlugin('Native Client', 'internal-nacl-plugin', '', []) ];
            const arr = Object.create(PluginArray.prototype); plugins.forEach((p, i) => Object.defineProperty(arr, i, { value: p })); Object.defineProperty(arr, 'length', { value: plugins.length }); Object.defineProperty(arr, 'item', { value: i => plugins[i] || null }); Object.defineProperty(arr, 'namedItem', { value: n => plugins.find(p => p.name === n) || null }); defProp(navProto, 'plugins', () => arr);
        } catch (e) {}
        try { const sp = Screen.prototype; defProp(sp, 'width', () => FP.screen.width); defProp(sp, 'height', () => FP.screen.height); defProp(sp, 'availWidth', () => FP.screen.width); defProp(sp, 'availHeight', () => FP.screen.height - 40); defProp(sp, 'colorDepth', () => FP.colorDepth); defProp(sp, 'pixelDepth', () => FP.colorDepth); } catch (e) {}
        try { const patchCtx = (ctx) => { if (!ctx) return ctx; try { const gp = ctx.getParameter.bind(ctx); Object.defineProperty(ctx, 'getParameter', { value: function (p) { if (p === 37445) return FP.gpuVendor; if (p === 37446) return FP.gpuRenderer; if (p === 7936) return 'WebKit'; if (p === 7937) return 'WebKit WebGL'; return gp(p); }, writable: false, configurable: false }); } catch (e) {} return ctx; }; const origGC = HTMLCanvasElement.prototype.getContext; HTMLCanvasElement.prototype.getContext = function (type, attrs) { const ctx = origGC.call(this, type, attrs); if (ctx && (type === 'webgl' || type === 'experimental-webgl' || type === 'webgl2')) { return patchCtx(ctx); } return ctx; }; } catch (e) {}
        try { const origResolved = Intl.DateTimeFormat.prototype.resolvedOptions; Intl.DateTimeFormat.prototype.resolvedOptions = function () { const r = origResolved.call(this); if (r.timeZone) r.timeZone = FP.timezone; return r; }; Date.prototype.getTimezoneOffset = function () { return FP.tzOffset; }; } catch (e) {}
        try { if (navigator.permissions && navigator.permissions.query) { const oq = navigator.permissions.query.bind(navigator.permissions); navigator.permissions.query = function (d) { if (d && d.name === 'notifications') return Promise.resolve({ state: 'default', onchange: null }); return oq(d); }; } } catch (e) {}
        try { window.chrome = window.chrome || {}; window.chrome.runtime = window.chrome.runtime || {}; window.chrome.app = window.chrome.app || { isInstalled: false }; window.chrome.csi = () => ({ onloadT: Date.now(), startE: Date.now(), pageT: 0, tran: 15 }); window.chrome.loadTimes = () => ({ commitLoadTime: Date.now()/1000, finishLoadTime: Date.now()/1000, navigationType: 'Other' }); } catch (e) {}
        try { const oRTC = window.RTCPeerConnection; window.RTCPeerConnection = function (cfg) { if (cfg && cfg.iceServers) cfg.iceServers = []; return new oRTC(cfg || { iceServers: [] }); }; } catch (e) {}
    })();
    `;
}
...
function makeBehaviorEngine() {
    ...
    async function moveMouse(page, tx, ty) {
        const sx = mouseX, sy = mouseY; const dist = Math.hypot(tx - sx, ty - sy); if (dist < 2) return;
    ...
    return { moveMouse, scrollDown, scrollUp, microMoves, pause };
}
...
/**
 * Wait for the currently playing video to end.
 * Returns: { ended: true, duration } or { ended: false, reason }
 */
async function waitForVideoEnd(page, B, log) {
    ...
    const info = await page.evaluate(() => {
        ...
        // Pick the largest/most prominent video
        let target = videos[0];
        let maxArea = 0;
        for (const v of videos) {
            const r = v.getBoundingClientRect();
            const area = r.width * r.height;
            if (area > maxArea && r.width > 100) {
                maxArea = area;
                target = v;
            }
        }
    ...
    log(`  Video found: ${info.count} videos | duration=${info.duration?.toFixed(1)}s | paused=${info.paused}`);

    if (!info.count || info.count === 0) {
        log(`  ! No video found, waiting 5s anyway`);
        await page.waitForTimeout(5000);
        return { ended: false, reason: 'no_video' };
    }

    // If video is paused (autoplay blocked), try to play it
    if (info.paused) {
    ...
    // Wait for ended event OR max duration
    const maxWait = Math.min(
        info.duration > 0 ? info.duration * 1000 : CFG.maxVideoDuration * 1000,
        CFG.maxVideoDuration * 1000
    ) + CFG.videoWaitBuffer * 1000;

    const startTime = Date.now();
    let ended = false;

    while (Date.now() - startTime < maxWait) {
        // Simulate human-like micro-interaction while watching
        if (Math.random() < 0.15) {
        ...
        const status = await page.evaluate(() => {
            const videos = Array.from(document.querySelectorAll('video'));
            if (videos.length === 0) return { ended: true, reason: 'video_removed' };
            let target = videos[0];
            let maxArea = 0;
            for (const v of videos) {
                const r = v.getBoundingClientRect();
            ...
            return {
                ended: target.ended || (target.duration > 0 && target.currentTime >= target.duration - 0.2),
    ...
    if (!ended) {
        log(`  ! Max wait reached (${Math.round(maxWait / 1000)}s)`);
    }

    // Small buffer after video ends (human reaction time)
...
/**
 * Move to next video.
 * Tries multiple methods: arrow keys, scroll, "next" button.
 */
async function goToNextVideo(page, B, log) {
    const method = CFG.nextVideoMethod === 'auto'
        ? pick(['scroll', 'keydown', 'scroll', 'keydown', 'button']) // weighted random
        : CFG.nextVideoMethod;

    log(`  → Next video via: ${method}`);

    try {
        if (method === 'scroll') {
            // Scroll down to trigger next video (TikTok style)
            await B.scrollDown(page, randInt(500, 1000));
            await page.waitForTimeout(randInt(1500, 3000));

        } else if (method === 'keydown') {
            // Press ArrowDown or PageDown
            const key = pick(['ArrowDown', 'ArrowDown', 'PageDown', 'j']);
            ...
            if (clicked) {
                ...
                log(`     No next button found, using keyboard fallback`);
...
async function runSession() {
    ...
    fp.timezone = tz; fp.locale = loc.locale; fp.languages = [...loc.languages];
    ...
    try {
        if (!isReturning) { try { fs.rmSync(sessionPath, { recursive: true, force: true }); } catch (e) {} }
        ...
        while (watched < targetVideos) {
            ...
            if (result.ended || result.duration > 0) {
                watched++;
                consecutiveFailures = 0;
                log(`  📺 Watched ${watched}/${targetVideos}`);
            } else {
                consecutiveFailures++;
                log(`  ⚠️  Video didn't end properly (failures: ${consecutiveFailures})`);
            ...
            if (Math.random() < 0.3) {
                ...
                log(`  Pause between videos: ${(hesitation / 1000).toFixed(1)}s`);
...
async function main() {
    console.log("╔═══════════════════════════════════════════════════╗");
    console.log("║   VIDEO TRAFFIC LAB v1.0 — Watch & Exit Random   ║");
    console.log("╚═══════════════════════════════════════════════════╝");
